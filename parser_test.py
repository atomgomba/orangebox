...
def compare(frame_index, parser, frame, csv_frame, show_all_fields):
    ...
    for i, value in enumerate(frame.data):
        ...
        if csv_value == value:
            if show_all_fields:
                stats.append(
                    "  field #{:d} '{:s}' value: {} encoding: {:d} predictor: {:d}".format(i, fdef.name, str(csv_value), fdef.encoding, fdef.predictor)
                )
            continue
        mismatched_num += 1
        stats.append(
            "  [!] field #{:d} '{:s}' value: {} != [{}] encoding: {:d} predictor: {:d}".format(
...
if __name__ == "__main__":
    ...
    argparser.add_argument("-i", "--index", type=int, dest="log_index", default=1, help="Log index number (In case of merged input)")
    argparser.add_argument("-a", "--show-all-fields", action="store_true", help="Show all fields of differing frames")
    argparser.add_argument("-v", dest="verbosity", action="count", default=0, help="Control verbosity (can be used multiple times)")
    ...
    logging.basicConfig(level=levels[args.verbosity], format="%(asctime)s %(name)s %(levelname)s %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
